...
def generate_focused_note(theme_name: str, files_data: list) -> str:
    ...
    for data in files_data:
        ...
        if ai_match:
            ...
            all_content.append(content[:2000])  # 限制长度
...
"""
    ...
    # 添加每个文件的核心要点
    for i, data in enumerate(files_data[:5], 1):  # 最多5个
        note += f"\n### {i}. {data['title'][:50]}\n\n"
        
        # 提取要点
        points = re.findall(r'[-*]\s*(.+?)(?:\n|$)', data['content'])
        for point in points[:5]:  # 每个文件最多5个要点
...
def main():
    ...
    # 4. 为每个主题生成聚焦笔记
    output_dir = VAULT_DIR / "03-Knowledge" / "主题整理"
    ...
    if created_files:
        ...
        log("  git add .")
        log("  git commit -m 'AI聚焦整理: 归纳碎片对话'")
        log("  git push")
