...
> **v2.1 更新**：入口D立即响应、日期编号+7天滚动、Dashboard自动更新
> 版本日期：2026-03-21
...
#### 5:00 原始对话收集（v2.1 修订版：日期编号 + 7天滚动）

**脚本**：`collect_raw_conversations.py`

**功能**：
- 收集过去24小时对话
- **v2.1 新增**：按日期编号保存：`00-Inbox/YYYY-MM-DD_raw.md`
- **v2.1 新增**：自动清理7天前的旧文件（滚动保留）
- **不分析、不AI处理**（仅收集原材料）

**v2.1 新增清理逻辑**：
```python
def cleanup_old_raw_files(inbox_dir, keep_days=7):
    """
    清理7天前的raw对话文件
    保留最近7天的记录，自动删除更早的文件
    """
    ...
    for file_path in Path(inbox_dir).glob("*_raw.md"):
...
**文件保留策略**：
- 格式：`YYYY-MM-DD_raw.md`
- 保留期：7天
- 自动删除：每天5:00任务执行时，自动清理7天前的文件

#### 8:30 复盘报告 + 整理提示（v2.1 修订版）

**脚本**：`daily_complete_report.py`

**功能**：
1. 生成复盘报告（现有功能）
2. **新增**：追加整理提示（使用v2.1日期编号格式）：

```
📋 今日对话已收集到 00-Inbox/YYYY-MM-DD_raw.md
...
queue_response_handler.add_pending(
    ...
    raw_file="00-Inbox/YYYY-MM-DD_raw.md",  # v2.1: 使用新命名格式
    timeout_minutes=15
)
```
...
```python
def handle_no_time_response(article_file, url):
    """
    ...
    # 设置待处理队列
    queue_response_handler.add_pending(
        type="article_auto_process_immediate",  # v2.1: 立即处理，无二次等待
        article_file=article_file,
        url=url,
        timeout_minutes=15
    )

def handle_auto_process_immediate(user_input, pending):
    """
    ...
    if user_input == "AI自动整理":
        ...
        # v2.1 新增：支持推迟到最新时间
        hours = parse_hours(user_input)  # 提取"推迟 X小时"中的X
        schedule_discussion(pending["article_file"], hours)
        return f"⏰ 已推迟 {hours} 小时，将在 {calculate_future_time(hours)} 再次提醒"
...
### 4.4 第四步：分类推送（v2.1 修订版：自动更新Dashboard）
...
2. 写入对应目录
3. **v2.1 新增**：自动更新Dashboard
   - 调用 `update_dashboard.py`
   - 统计各类笔记数量
   - 更新README中的统计信息
4. Git提交：
```bash
...
**v2.1 Dashboard自动更新实现**：
```python
def push_to_github_with_dashboard(files_created, source_file):
    """
    v2.1 修订版：推送GitHub时自动更新Dashboard
    """
    # 1. 写入所有文件到对应目录
    for file_path, content in files_created:
        write_file(file_path, content)
    
    # 2. v2.1 新增：自动更新Dashboard
    dashboard_result = update_dashboard()
    print(f"[INFO] Dashboard 更新状态: {dashboard_result}")
    
    # 3. Git提交
    subprocess.run(["git", "add", "-A"], check=True)
    subprocess.run(["git", "commit", "-m", f"discussions: 提取主题讨论精华\n\n..."], check=True)
    ...
    生成主题讨论精华：
    {format_file_list(files_created)}
...
| `scheduled_discussion_handler.py` | 入口D：定时任务自动处理 | 用户回复"没有时间"后触发 |
