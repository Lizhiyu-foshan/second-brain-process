...
消息是否重复？ ──是──→ 规则1（NO_REPLY）
  ↓ 否
涉及凭据/Token/密码？ ──是──→ 规则3（凭据处理优先）
  ↓ 否
是 git push 操作？ ──是──→ 规则3（GitHub推送规范）
  ↓ 否
是代码/脚本修改？ ──是──→ 规则4（代码部署全流程）
  ↓ 否
是定时任务配置？ ──是──→ 规则5（定时任务监控）
  ↓ 否
是模型切换？ ──是──→ 规则6（模型切换检查）
  ↓ 否
是高风险操作？ ──是──→ 规则2（高风险操作审查）
  ↓ 否
正常处理
```
...
### 规则4：代码部署全流程
**Priority**: critical | **Status**: active  
**关联规则**: 规则3（推送前凭据检查）、规则2（风险审查）、通用检查清单
...
**审计方式**：
- **首选模型**：`qwen/qwen3.5-coder-plus`（速度快、成本低）
...
请审查以下代码：
1. 代码逻辑是否合理，有无明显 bug
2. 边界情况是否处理（空值、超大文件、网络失败）
3. 错误处理是否完善（异常捕获、回滚机制）
4. 性能是否有问题（循环、递归、资源泄漏）
5. 是否符合项目代码风格
...
- 【PASS】→ 继续部署流程
- 【FAIL】→ 必须修复问题后才能部署
- 审计超时（>5分钟）→ 切换到 k2p5 重试

**3. 端对端测试**
- 单元测试：函数可正常导入，无语法错误
- 集成测试：完整流程至少跑一遍，检查输出是否符合预期
- 定时任务测试（如相关）：检查下次执行时间，模拟执行一次
...
### 规则5：定时任务配置与监控
**Priority**: critical | **Status**: active  
**关联规则**: 规则1（消息发送去重）、规则4（脚本部署流程）、通用检查清单
...
| 场景 | 触发规则 | 执行顺序 | 说明 |
|------|---------|---------|------|
| 修改定时任务脚本并部署 | 规则4 + 规则5 | 先4后5 | 先部署验证，再配置监控 |
| 修改GitHub相关代码 | 规则3 + 规则4 | 先3后4 | 凭据检查优先 |
| 模型切换后部署 | 规则6 + 规则4 | 先6后4 | 环境就绪再部署 |
| 定时任务涉及git push | 规则1 + 规则3 + 规则5 | 1→3→5 | 去重→凭据→任务 |
| AGENTS.md 修改 | 规则3 + 规则4 | 先3后4 | 先备份再修改 |
...
|------|---------|---------|---------|
| 规则1 | 无 | 规则5（发送时） | 消息处理 |
| 规则2 | 无 | 规则3/4/5/6 | 高风险操作 |
| 规则3 | 规则2（如高风险） | 规则4（推送后部署） | Git操作 |
| 规则4 | 规则3（如需推送）、通用清单 | 无 | 代码部署 |
| 规则5 | 规则1（发送去重）、通用清单 | 无 | 定时任务 |
| 规则6 | 规则2（如高风险） | 规则4（切换后部署） | 模型切换 |

### 修订历史

| 日期 | 版本 | 变更说明 |
|------|------|---------|
| 2026-03-19 | v2.0 | 重构版本：新增快速决策入口、通用检查清单、优先级矩阵；合并规则4+6为规则3；简化规则4、5的内容 |
| 2026-03-18 | v1.7 | 添加规则7（定时任务配置与监控） |
| 2026-03-18 | v1.6 | 添加规则6（凭据处理禁令） |
| 2026-03-12 | v1.5 | 添加规则1（消息接收去重检查） |
| 2026-03-12 | v1.4 | 添加规则4（GitHub推送规范） |
| 2026-03-10 | v1.3 | 合并GitHub推送相关规则 |
| 2026-03-08 | v1.2 | 添加规则2（模型切换检查）、规则3（高风险操作审查） |
| 2026-03-05 | v1.1 | 添加规则5（代码部署全流程） |
