...
def is_fragment_file(filepath: Path) -> bool:
    """判断是否为碎片文件（需要删除的）"""
...
def main():
    log("=" * 50)
    log("AI 深度整理 - 碎片对话聚焦（修正版）")
    log("=" * 50)
    
    # 1. 获取最近3天的对话文件（只处理这些，删除碎片）
    cutoff_date = datetime.now() - timedelta(days=3)
    recent_files = []
    fragment_files_to_delete = []
    
    for filepath in CONVERSATIONS_DIR.glob("*.md"):
        mtime = datetime.fromtimestamp(filepath.stat().st_mtime)
        if mtime >= cutoff_date:
            if is_fragment_file(filepath):
                fragment_files_to_delete.append(filepath)
                log(f"  [碎片] {filepath.name} - 将删除")
            else:
                recent_files.append(filepath)
    
    log(f"\n最近3天文件统计:")
    log(f"  - 保留文件: {len(recent_files)} 个")
    log(f"  - 碎片文件（将删除）: {len(fragment_files_to_delete)} 个")
    ...
    # 5. 删除碎片文件
    deleted_files = []
    for filepath in fragment_files_to_delete:
        try:
            filepath.unlink()
            deleted_files.append(filepath.name)
            log(f"  🗑️ 删除: {filepath.name}")
        except Exception as e:
            log(f"  ⚠️ 删除失败 {filepath.name}: {e}")
    ...
    log(f"  - 删除碎片文件: {len(deleted_files)}")
    log(f"  - 保留其他日期文件: ✅")
    log("=" * 50)
