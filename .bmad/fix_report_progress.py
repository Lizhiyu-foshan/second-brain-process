#!/usr/bin/env python3
"""
修复 run_daily_report_progress.py 的 HIGH 问题
"""

import re
from pathlib import Path

FILE_PATH = Path("/root/.openclaw/workspace/second-brain-processor/run_daily_report_progress.py")
...
def add_exception_handling(content):
    ...
    while i < len(lines):
        ...
        if 'urllib.request.urlopen' in line or 'requests.' in line:
            in_try_block = False
            for j in range(max(0, i-15), i):
                if 'try:' in lines[j]:
                    in_try_block = True
